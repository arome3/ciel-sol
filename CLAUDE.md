...
6. **Off-chain and on-chain Borsh serialization must agree on the wire bytes for all three attestation types.** Committed fixtures pin the exact wire format for each type:
   - `crates/ciel-signer/fixtures/ciel_attestation_v1.bin` (132 bytes)
   - `crates/ciel-signer/fixtures/policy_attestation_v1.bin` (86 bytes)
   - `crates/ciel-signer/fixtures/override_attestation_v1.bin` (86 bytes)
   - `crates/ciel-signer/fixtures/attestation_fixtures.json` — JSON manifest documenting every field value used to generate the binary fixtures.

   Both `ciel-signer` (off-chain, borsh 1.x) and `ciel-assert` (on-chain, Anchor/borsh) must produce and consume identical bytes. The `test_*_wire_fixture_v1` and `test_*_wire_fixture_v1_roundtrip` tests in `crates/ciel-signer/src/attestation.rs` catch drift between the JSON manifest, Rust struct field order, and committed binary bytes. When implementing Unit 20 (CielAssert program), add matching tests that deserialize these same fixture files and assert structural equality with the JSON manifest values. To regenerate the fixtures after a deliberate manifest change, run: `cargo test --package ciel-signer write_attestation_v1_fixtures -- --ignored --nocapture`.
